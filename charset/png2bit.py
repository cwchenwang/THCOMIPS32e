...
for file in os.listdir(os.getcwd() + "/charset"):
    ...
    for j in range(im.size[1]):
      for i in range(im.size[0]):
            ...
            binary_code = "000000000000000000000000" + toBit(r, 3) + toBit(g, 3) + toBit(b, 2)
            #if(r>100):
            ...
            data = data + int(binary_code, 2).to_bytes(4, byteorder="little")
    char_code[numId]=data

bit_file = open("charset.bit", "wb")
for x in range(128):
  if(x in char_code):
    if(len(char_code[x])!=dx*dy*4):
    ...
    for i in range(dx):
      for j in range(dy):
        bit_file.write(int(0).to_bytes(4,byteorder='little'))
bit_file.close()