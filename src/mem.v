...
module mem(
	...
	input wire[`ByteWidth]		 base_ram_data
);
	...
	always @ (*) begin
		if(rst == `RstEnable) begin
		  ...
		  cp0_reg_data_o <= cp0_reg_data_i;		 		  	
			case (aluop_i)
				`EXE_LB_OP:		begin
					// for serial port load
					if(mem_addr_i == 8'hBFD003F8) begin
						wdata_o <= {32{base_ram_data}};
						mem_ce_o <= `ChipDisable;
					end else if(mem_addr_i == 8'hBFD003FC) begin
						wdata_o <= 8`h00000000 + (tbre & tsre) + (tbre & tsre) + (data_ready);
						mem_ce_o <= `ChipDisable;
					end else begin
				...
				`EXE_SB_OP:		begin
					// for serial port store
					if(mem_addr_i == 8'hBFD003F8) begin
						base_ram_data <= reg2_i[7:0];
						mem_ce_o <= `ChipDisable;
					end else if(mem_addr_i == 8'hBFD003FC) begin	// not exist
						mem_ce_o <= `ChipDisable;
					end else begin
						mem_addr_o <= mem_addr_i;
						mem_we <= `WriteEnable;
